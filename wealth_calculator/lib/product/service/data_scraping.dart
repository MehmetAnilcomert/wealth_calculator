...
WealthPrice? _parseBigparaRow(dom.Element row, PriceType type, String time) {
  ...
  final title = titleDivs.isNotEmpty ? titleDivs.first.text.trim() : cells[0].text.trim();

  if (title.isEmpty) return null;

  // --- Buying & Selling ---
  final buyingPrice = cells[5].text.trim();
  final sellingPrice = cells[6].text.trim();
    ...
    if (el.text.contains('Son Güncelleme') && el.children.isEmpty) {
      ...
      if (parent != null) {
        ...
        if (idx >= 0 && idx + 1 < siblings.length) {
          return siblings[idx + 1].text.trim();
        }
      }
    }
  }
